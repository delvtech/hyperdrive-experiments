...
def trade_long(interface:HyperdriveReadWriteInterface, agent:LocalHyperdriveAgent, trade_size):
    try:
        ...
        base_traded = trade_result.args.amount
        bonds_traded = trade_result.args.bond_amount
...
def trade_short(interface:HyperdriveReadWriteInterface, agent:LocalHyperdriveAgent, trade_size):
    try:
        ...
        base_traded = -trade_result.args.amount
        bonds_traded = -trade_result.args.bond_amount
        price, rate = calc_price_and_rate(interface)
        return price, rate, base_traded, bonds_traded
    except:
        pass
    return None, None, None, None
...
start_liq = FixedPoint(100)
liquidity = FixedPoint(os.environ["LIQUIDITY"]) if "LIQUIDITY" in os.environ else FixedPoint(140)*start_liq/FixedPoint(100)

# %%
# params
STARTING_RATE = 0.0357
TIME_STRETCH_APR = 0.04  # is not cancelled out in the time stretch formula
TERM_YEARS = 10  # cancelled out in the time stretch formula
POSITION_DURATION = int(YEAR_IN_SECONDS * TERM_YEARS)
...
interactive_config = LocalHyperdrive.Config(
    ...
    initial_fixed_apr=FixedPoint(STARTING_RATE),
...
spot_rate = hyperdrive.interface.calc_spot_rate()
print(f"market deployed! spot rate = {float(spot_rate):.5%}")
print(f" {TERM_YEARS=}")
chain.save_snapshot()
...
# add liquidity
TOTAL_LP = FixedPoint(25e6)
AMOUNT_TO_LP = TOTAL_LP-start_liq  # how much to trade each time
agent0.add_liquidity(base=AMOUNT_TO_LP)
spot_rate = hyperdrive.interface.calc_spot_rate()
pool_info = hyperdrive.interface.current_pool_state.pool_info
print(f"liquidity added! spot rate = {float(spot_rate):.5%}")
for k,i in pool_info.__dict__.items():
    print(f" {k:<31} = {i}")

# we do a little short
# trade_size = hyperdrive.interface.calc_targeted_long(budget=FixedPoint(1e18), target_rate=FixedPoint(TARGET_RATE))
max_open_short = hyperdrive.interface.calc_max_short(budget=FixedPoint(1e18))
print(f"{max_open_short=}")
trade_result = agent1.open_short(bonds=FixedPoint(5e6))
spot_rate = hyperdrive.interface.calc_spot_rate()
base_proceeds = -trade_result.args.base_proceeds
bonds_traded = -trade_result.args.bond_amount
ending_price, ending_rate = calc_price_and_rate(hyperdrive.interface)
print(f"shorted! new spot rate = {float(spot_rate):.5%}")
print(f" base proceeds = {base_proceeds}")
print(f" bonds traded = {bonds_traded}")

# calc effective rate
effective_price = base_proceeds/bonds_traded
print(f"{effective_price=}")
effective_apr = (1-float(effective_price))/float(effective_price) / float(TERM_YEARS)
print(f"{effective_apr=}")
