...
def plot_utilization_and_rates(_irm: AaveIRM, utilization, borrow_rates, supply_rates=None, effective_rates=None, rate_at_target_list=None, file_suffix=''):
...
def plot_transformation_function(_irm: AaveIRM, utilization):
...
# %%
def calculate_irm_stats(_irm: AaveIRM, utilization_list, utilization_gap_point = 0.35, hd_fixed_rate=None) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
# HD fixed rates
irm = AaveIRM(u_target=0.92, slope_1=CURRENT_RATE_AT_TARGET, slope_2=0.35, reserve_factor=0.10, base_rate=0)
if USE_QUOTED_RATE:
